...
/* USER CODE BEGIN PV */
char BufferTransmit[130] ;// Buffer for
char BufferRec[130];
char sendCntrlCMD = 0;
char switchOffCMD = 0;
char TXcomplete = 0;
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart1,(uint8_t *)BufferRec,130);	 // start the receive here.
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if (sendCntrlCMD){
		...
		HAL_UART_Transmit_IT(&huart1, (uint8_t *)&BufferTransmit, 3);
		sendCntrlCMD = 0;
	  }

	  if (cmd == 1){
		sprintf(BufferTransmit,"D\n");
		HAL_UART_Transmit_IT(&huart1, (uint8_t *)&BufferTransmit, 2);
		cmd = 0;
	  }


	  if (cmd == 2){
		sprintf(BufferTransmit,"E\n");
		HAL_UART_Transmit_IT(&huart1, (uint8_t *)&BufferTransmit, 2);
		cmd = 0;
	  }

	  if (cmd == 3){
		sprintf(BufferTransmit,"SN,Flyer01\n");
		HAL_UART_Transmit_IT(&huart1, (uint8_t *)&BufferTransmit, 13);
		cmd = 0;
	}
